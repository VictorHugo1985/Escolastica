  ...
  try {
    ...
    const clase = await prisma.clases.findUnique({
      ...
      include: {
        ...
        inscripciones: { orderBy: [{ estado: 'asc' }, { fecha_inscripcion: 'asc' }], include: { usuario: { select: { id: true, nombre_completo: true, email: true } }, notas_finales: { select: { id: true, tipo_nota: true, valor: true }, orderBy: { created_at: 'asc' } } } },
