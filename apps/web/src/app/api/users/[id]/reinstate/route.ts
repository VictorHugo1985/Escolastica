...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!user.roles.some((r) => r.rol.nombre === 'ExProbacionista')) throw new ApiError('El usuario no tiene el rol ExProbacionista', 400);
    ...
    if (!exProbRol || !probRol) throw new ApiError('Roles no encontrados en el sistema', 400);

    await prisma.$transaction([
      prisma.usuario_roles.delete({ where: { usuario_id_rol_id: { usuario_id: id, rol_id: exProbRol.id } } }),
      prisma.usuario_roles.create({ data: { usuario_id: id, rol_id: probRol.id, asignado_por_id: actor.sub } }),
